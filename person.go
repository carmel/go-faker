...
type Dowser interface {
	...
	SSN(v reflect.Value) (interface{}, error)
}
...
var chineseFirstNameMale = []string{
...
var chineseFirstNameFemale = []string{
...
var chineseFirstNames = append(chineseFirstNameMale, chineseFirstNameFemale...)
...
var chineseLastNames = []string{
	...
	"汤", "向", "常", "温", "康", "施", "文", "牛", "樊", "葛", "邢", "安", "齐", "易", "乔", "伍", "庞", "颜", "倪", "庄", "聂", "章", "鲁", "岳", "翟", "殷", "詹", "申", "欧", "耿", "关", "兰", "焦", "俞", "左", "柳", "甘", "祝", "包", "宁", "尚", "符", "舒", "阮", "柯", "纪", "梅", "童", "凌", "毕", "单", "季", "裴", "霍", "涂", "成", "苗", "谷", "盛", "曲", "翁", "冉", "骆", "蓝", "路", "游", "辛", "靳", "欧阳", "管", "柴", "蒙", "鲍", "华", "喻", "祁", "蒲", "房", "滕", "屈", "饶", "解", "牟", "艾", "尤", "阳", "时", "穆", "农", "司", "卓", "古", "吉", "缪", "简", "车", "项", "连", "芦", "麦",
	"褚", "娄", "窦", "戚", "岑", "景", "党", "宫", "费", "卜", "冷", "晏", "席", "卫", "米", "柏", "宗", "瞿", "桂", "全", "佟", "应", "臧", "闵", "苟", "邬", "边", "卞", "姬", "师", "和", "仇", "栾", "隋", "商", "刁", "沙", "荣", "巫", "寇", "桑", "郎", "甄", "丛", "仲", "虞", "敖", "巩", "明", "佘", "池", "查", "麻", "苑", "迟", "邝", "官", "封", "谈", "匡", "鞠", "惠", "荆", "乐", "冀", "郁", "胥", "南", "班", "储", "原", "栗", "燕", "楚", "鄢", "劳", "谌", "奚", "皮", "粟", "冼", "蔺", "楼", "盘", "满", "闻", "位", "厉", "伊", "仝", "区", "郜", "海", "阚", "花", "权", "强", "帅", "屠", "豆",
	"朴", "盖", "练", "廉", "禹", "井", "祖", "漆", "巴", "丰", "支", "卿", "国", "狄", "平", "计", "索", "宣", "晋", "相", "初", "门", "云", "容", "敬", "来", "扈", "晁", "芮", "都", "普", "阙", "浦", "戈", "伏", "鹿", "薄", "邸", "雍", "辜", "羊", "阿", "乌", "母", "裘", "亓", "修", "邰", "赫", "杭", "况", "那", "宿", "鲜", "印", "逯", "隆", "茹", "诸", "战", "慕", "危", "玉", "银", "亢", "嵇", "公", "哈", "湛", "宾", "戎", "勾", "茅", "利", "于", "呼", "居", "揭", "干", "但", "尉", "冶", "斯", "元", "束", "檀", "衣", "信", "展", "阴", "昝", "智", "幸", "奉", "植", "衡", "富", "尧", "闭", "由",
}
...
func checkSum(s string) string {
	i := new(big.Int)
	i.SetString(s, 13)
	return strings.ReplaceAll(strconv.FormatInt((1-2*i.Int64())%11, 13), "10", "X")
}

func (p Person) ssn() string {
	// rand.Seed(time.Now().UnixNano())

	// min := now + 18*365*24*60*60
	// max := now + 90*365*24*60*60
	// birthday := time.Unix(rand.Int63n(max-min+1)+min, 0).Format("20060102")

	// ssn_without_checksum = self.numerify(
	//     self.random_element(self.area_codes) + birthday + "##")
	...
	// return randomElementFromSliceString()
	return ""
}

// TitleMale generates random titles for males
func (p Person) SSN(v reflect.Value) (interface{}, error) {
	return p.ssn(), nil
}

// TitleMale get a title male randomly in string ("Mr.", "Dr.", "Prof.", "Lord", "King", "Prince")
func SSN() string {
	return singleFakeData(SSNTag, func() interface{} {
		p := Person{}
		return p.ssn()
	}).(string)
}
